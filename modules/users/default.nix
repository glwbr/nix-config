...
in {
  ...
  config = lib.mkMerge [
    (lib.mkIf cfg.enable {
      ...
      # users.users = lib.mkIf cfg.root.enable {
      #   root = {
      #     shell = cfg.defaultUserShell;
      #     extraGroups = ifTheyExist cfg.root.extraGroups;
      #   } // lib.optionalAttrs (cfg.root.hashedPassword != null) {
      #     inherit (cfg.root) hashedPassword;
      #   } // lib.optionalAttrs (cfg.root.sshKeys != []) {
